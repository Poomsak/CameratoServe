...
public class PjpegServer implements Runnable {
    ...
    public void run() {
        ...
        while(true) {
            ...
            try {
                ...
                Log.e("PjpegServer", "RunSerVer");
                Socket socket = server.accept();
                if (allIpsAllowed || socket.getInetAddress().isSiteLocalAddress()) {
                    PjpegSocket pjpegSocket = new PjpegSocket(socket);

                    new Thread(pjpegSocket).start();
                } else {
                    socket.close();
                }
